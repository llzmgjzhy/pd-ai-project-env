...
def pred(args, model, tst_dataset, filename, staname):
    ...
    with torch.no_grad():
        for inputs in tst_loader:
            ...
            prob = torch.max(torch.sigmoid(outputs))
...
def main(args):
    ...
    info_list_prpd = read_us_data_info(table_name="us_waveform_prpd_info")
    info_list_prps = read_us_data_info(table_name="us_waveform_prps_info")
    info_list = pd.merge(
    ...
    for i in range(len(info_list)):
        ...
        device_id = re.findall(r"\d+", staname)[0]
        if device_id == "614":
            staname = "614开关柜"
        else:
            staname = "620开关柜"

        filenames = get_filenames_jump_one(info_list["FILE_NAME"], i)
